...
`;
  ...
  try {
      ...
      body: JSON.stringify({
       ...
       model: "mistralai/mistral-7b-instruct", // ✅ use a working free model
        ...
        messages: [
          {
            role: "system",
            content: "You are a motivational and intelligent AI study coach."
          },
          {
            role: "user",
            content: prompt
    ...
    const data = await response.json();
    console.log("📦 OpenRouter Response (Today Suggestion):", data);

    return data.choices?.[0]?.message?.content || "❌ AI failed to generate suggestions.";
  } catch (err) {
    console.error("❌ Error in generateTodaySuggestion:", err);
